...
//        dd($result);
    ...
    public function platform_wise_report($filter =[], $size = 100){
            $trackingKeyword = static::select('id', 'publisher_job_id', 'publisher_id', 'campaign_id', 'advertiser_id', 'subid', 'ip', 'date', DB::raw('case when  (`is_tablet`=1) then "tablet" when (`is_mobile`=1) then "mobile" else `platform` end as platform') , DB::raw('count(*) as total_count'));
